...
- Correction after user feedback (2026-09-17): the flow accent is now one uniform `box-shadow` ring (1.5px accent + soft glow) instead of the masked gradient pseudo-element that read thinner at the bottom edge; the sidebar Gentle AI utility button uses the rose `GentleWordmark` in brand pink with a pink hover tint. Real-client check on the built bundle (isolated `T3CODE_HOME`, port 13911, this repo added as a project): the readiness banner renders ("partially set up … Missing the SDD workspace (openspec/config.yaml)") with both actions; clicking "Set up with SDD init" switches the picker to Init and applies `gentle-flow-frame--sdd` with computed `box-shadow` `0 0 0 1.5px` + `0 0 26px -4px`; `::before` content is `none`; the sidebar button renders the 64-unit rose SVG in `rgb(240,149,200)`. The user's "no banner" report matches a server process started before the `openspecConfigPresent` change (the client cannot decode the older response); restarting the server resolves it.
